...
if type java > /dev/null && type javac > /dev/null; then
	...
	if [ -d "/opt/homebrew/opt/openjdk@$JAVA_MAJOR_VERSION" ]; then
		export PATH="/opt/homebrew/opt/openjdk@$JAVA_MAJOR_VERSION/bin:$PATH"
		export JAVA_HOME="/opt/homebrew/Cellar/openjdk/$JAVA_VERSION/libexec/openjdk.jdk/Contents/Home"
		export UPDATE_JAVA=true
	fi
